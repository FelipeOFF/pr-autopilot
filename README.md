...
| **Cascade** | `/pr-autopilot --cascade` | IDs → a forest of work items. Reuse an open work-item PR; halt the forest on failure. No IDs and current PR base ≠ trunk → existing-chain (path to this PR; siblings stay). Independent items are roots against the trunk. A blocked child stacks on the parent head. Current feature branch is not the parent. `--auto` does not turn this on. |
