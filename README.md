...
| **Cascade** | `/pr-autopilot --cascade` | Forest of GitHub work items. Independent items are roots against the trunk. A blocked child stacks on the parent head. Current feature branch is not the parent. `--auto` does not turn this on. |
...
# Forest of GitHub work items against the trunk
...
| `--cascade` | `false` | Forest of GitHub work items. Independent items are roots against the trunk. A blocked child stacks on the parent PR head. Not implied by `--auto`. A phrase like "cascade these tickets" sets it. |
