...
| **Cascade** | `/pr-autopilot --cascade` | Pick tracker source from this repo or IDs, then a forest of work items. Independent items are roots against the trunk. A blocked child stacks on the parent head. An open work-item PR is reused (restacked if the base is wrong). A failed ship stops the forest. Current feature branch is not the parent. `--auto` does not turn this on. |
...
| `--cascade` | `false` | Pick tracker source from this repo or IDs, then a forest of work items. Independent items are roots against the trunk. A blocked child stacks on the parent PR head. Reuses an open work-item PR (restacks if the base is wrong). A failed ship stops the forest. Not implied by `--auto`. A phrase like "cascade these tickets" sets it. |
