...
| **Cascade** | `/pr-autopilot --cascade` | IDs → a forest of work items. No IDs and current PR base ≠ trunk → existing-chain (path to this PR; siblings stay). Independent items are roots against the trunk. A blocked child stacks on the parent head. Current feature branch is not the parent. `--auto` does not turn this on. |
...
| `--cascade` | `false` | Opt-in. IDs or "these tickets": a forest. No IDs and current PR base ≠ trunk: existing-chain, the path from the trunk to this PR; siblings stay off the path. Independent items are roots against the trunk. A blocked child stacks on the parent PR head. Not implied by `--auto`. A phrase like "cascade these tickets" sets it. |
