...
(`--auto`, `--review`, `--resolve`, `--merge`, `--show-me`, `--cascade`, `--draft`, …) aparecem
...
- `--auto` **não** implica `--cascade`. Uma floresta empilhada é um opt-in separado. Uma frase como "cascade these tickets" liga `--cascade`; `--auto` não liga.
