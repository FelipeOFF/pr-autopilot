...
| **Cascade** | `/pr-autopilot --cascade` | Escolhe o source do tracker neste repo ou nos IDs, depois uma floresta de work items. Itens independentes são raízes contra o trunk. Um filho bloqueado empilha no head do parent. O feature branch atual não é o parent. `--auto` não liga isso. |
...
| `--cascade` | `false` | Escolhe o source do tracker neste repo ou nos IDs, depois uma floresta de work items. Itens independentes são raízes contra o trunk. Um filho bloqueado empilha no head do PR parent. `--auto` não implica. Uma frase como "cascade these tickets" liga. |
