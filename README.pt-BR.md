...
| **Cascade** | `/pr-autopilot --cascade` | IDs → uma floresta de work items. Sem IDs e a base do PR atual ≠ trunk → existing-chain (path até este PR; irmãos ficam de fora). Itens independentes são raízes contra o trunk. Um filho bloqueado empilha no head do parent. O feature branch atual não é o parent. `--auto` não liga isso. |
...
| `--cascade` | `false` | Opt-in. IDs ou "these tickets": uma floresta. Sem IDs e a base do PR atual ≠ trunk: existing-chain, o path do trunk até este PR; irmãos ficam fora do path. Itens independentes são raízes contra o trunk. Um filho bloqueado empilha no head do PR parent. `--auto` não implica. Uma frase como "cascade these tickets" liga. |
