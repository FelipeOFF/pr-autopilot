...
- **`--cascade` work items contra o trunk** — opt-in. Escolhe o source do tracker neste repo ou nos IDs do prompt (GitHub se origin for GitHub, GitLab se GitLab, beads se `.beads/` existir, Jira só com um projeto do repo ou `PROJ-123` no prompt). MCP instalado globalmente não é source. `#9` / `PROJ-12` / um id `bd` escolhem o source sem perguntar. Dois sources, zero sources, ou um prompt que é só o ID de um spec: pergunta uma vez; com `--auto`, para. Tickets Cairn são beads. Sem arquivo de config de tracker. Implementa um work item desbloqueado e abre um PR contra o trunk (`--base`, ou o default do repo). `--auto` não liga isso. Uma frase como "cascade these tickets" liga. O feature branch atual não é o parent. `ready-for-human` é pulado a menos que o ID tenha sido nomeado. `--title` / `--body` não são carimbados no PR do work item.
...
| **Cascade** | `/pr-autopilot --cascade` | Escolhe o source do tracker neste repo ou nos IDs, depois um work item desbloqueado como PR contra o trunk. O feature branch atual não é o parent. `--auto` não liga isso. |
...
# Escolhe o source neste repo ou nos IDs, depois um work item
# desbloqueado como PR contra o trunk (--auto não implica isso)
/pr-autopilot --cascade

# A mesma flag via frase: cascade these tickets

# IDs escolhem o source sem perguntar
...
| `--cascade` | `false` | Escolhe o source do tracker neste repo ou nos IDs, depois envia um work item desbloqueado como PR contra o trunk. `--auto` não implica. Uma frase como "cascade these tickets" liga. |
