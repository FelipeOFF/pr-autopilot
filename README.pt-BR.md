...
**Opt-in por padrão.** Todo estágio fica desligado até você pedir. O `pr-autopilot` puro só abre o PR e para. Você liga cada estágio com uma flag (`--review`, `--resolve`, `--merge`) ou liga todos de uma vez com `--auto`. `--cascade` é um opt-in separado: escolhe o source do tracker neste repo ou nos IDs do prompt e envia uma floresta de work items como PRs empilhados. Itens independentes são raízes contra o trunk; um filho bloqueado empilha no head do parent. Um PR aberto do work item é reutilizado; uma falha para a floresta. `--auto` não liga isso.
...
| **Cascade** | `/pr-autopilot --cascade` | Escolhe o source do tracker neste repo ou nos IDs, depois uma floresta de work items. Itens independentes são raízes contra o trunk. Um filho bloqueado empilha no head do parent. Um PR aberto do work item é reutilizado (restack se a base estiver errada). Uma falha para a floresta. O feature branch atual não é o parent. `--auto` não liga isso. |
...
| `--cascade` | `false` | Escolhe o source do tracker neste repo ou nos IDs, depois uma floresta de work items. Itens independentes são raízes contra o trunk. Um filho bloqueado empilha no head do PR parent. Reutiliza um PR aberto do work item (restack se a base estiver errada). Uma falha para a floresta. `--auto` não implica. Uma frase como "cascade these tickets" liga. |
